...
## Requirements
Note that we support an Exasol database version 7.1 or later. Ensure your Exasol instance meets this minimum version requirement before proceeding.
...
Alternatively, grant system privileges to allow access to all schemas and tables:
...
- **SSL/TLS Settings**: Specifies the mode or settings for SSL/TLS validation during the connection. Available options:
  ...
  - **disable-tls**: Disables Transport Layer Security (TLS). Data is sent in plain text (no encryption).
    <Tip>
    This mode is **never** recommended for production and should only be used in debugging scenarios.
    </Tip>
</Step>
